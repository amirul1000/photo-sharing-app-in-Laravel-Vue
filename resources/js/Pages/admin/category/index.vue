...
<div class="card">
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            <h1>Category</h1>
            <router-link to="/category/add" class="btn btn-secondary"
                >Create</router-link>
        </div>
    </div>
    <div class="card-body">
	   ...
	   <!--Action-->
        ...
        <table class="table table-bordered table-hover">
            ...
            <tbody>
                <tr v-for="(cat, key) in category.data" :key="cat.id">
                    <td>{{ key }}</td>
                    <td>{{ cat.cat_name }}</td>
                    ...
                    <td>
                        ...
                        <button
                            ...
                            v-on:click="deleteCategory(cat.id)">
                            Delete
                        </button>
                    </td>
                </tr>
  ...
  <Bootstrap5Pagination align="center" :data="category" :limit="2" :size="default"  @pagination-change-page="getCategory"></Bootstrap5Pagination>
...
export default {
    ...
    methods: {
        async getCategory(page=1) {
		     const url = `/api/admin/category/index?page=${page}`;		     
			 this.show_load = true;
        ...
        async deleteCategory(id) {
		    if(confirm('Are you sure to delete this item?')){
				let res = await axios.get(`/api/admin/category/destroy/${id}`);
		...
		async searchData(page=1) {	
		      ...
		      const value = this.search;
		        this.show_load = true;
                await axios.get(`/api/admin/category/search?page=${page}&key=${value}`).then(({data})=>{
                    this.category = data;
					this.show_load = false;
                }).catch(({ response })=>{
                    console.error(response)
		 ...
		 exportData() {
		       window.open('/api/admin/category/'+this.exporttype,'_blank');
        },
